 ...
 */
definition(
    name: "SmartPi Device Manager",
    namespace: "randuhmm",
    author: "Jonny Morrill",
    description: "A device manager application for SmartPi devices.",
    category: "My Apps",
...
preferences {
    page(name: "searchTargetSelection", title: "UPnP Search Target", nextPage: "deviceDiscovery") {
        section("Search Target") {
            input "searchTarget", "string", title: "Search Target", defaultValue: "urn:Randuhmm:device:hub:1", required: true
        }
    }
...
def addDevices() {
    ...
    selectedDevices.each { dni ->
        ...
        if (!d) {
            ...
            addChildDevice("randuhmm", "SmartPi", selectedDevice.value.mac, selectedDevice?.value.hub, [
                "label": selectedDevice?.value?.name ?: "SmartPi",
                "data": [
            ...
            selectedDevice?.value?.children.each { child ->
            	def child_dni = "${selectedDevice.value.mac}-${child.id}"
                addChildDevice("randuhmm", child.type, child_dni, selectedDevice?.value.hub, [
...
void deviceDescriptionHandler(physicalgraph.device.HubResponse hubResponse) {
    ...
    if (device) {
        device.value << [
            name: body.device.deviceName,
            model: body.device.modelName,
            serialNumber: body.device.serialNum,
            verified: true,
            children: body.device.children
        ]
    }
}
...
def dispatchEvent(childId, parsedEvent, rawEvent) {
	String dni = "${parsedEvent.mac}-$childId"
